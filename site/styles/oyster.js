...
Oyster Javascript Library

These are basically shortcuts to common MooTools tasks.

Most functions are available on all pages, however, some are called via oyster/js.xsl.

This is part of Oyster and falls under the same licensing terms.
...
Prototype:
	oyster.toggle_visibility(string some_element_id)
*/
...
Prototype:
	oyster.toggle_checkbox(some_checkbox_element_id)
*/
...
ajax: {
	...
	Prototype:
		oyster.ajax.popup(string url)
	*/

	popup: function(url) {

		// show the popup background overlay
		$('oyster_ajax_popup_overlay').setStyle('height', window.getScrollHeight()) + window.getHeight();
		$('oyster_ajax_popup_overlay').setStyle('display', 'block');


		// display the popup
		$('oyster_ajax_popup').setStyle('display', 'block');

		// set the popup's content to the loading message
		$('oyster_ajax_popup_content').innerHTML = $('oyster_ajax_popup_loading').innerHTML;

		// center the popup
		oyster.ajax.center_popup();

		// send the ajax request
		oyster.ajax.submit_url(url, 'oyster_ajax_popup_content');
	},
	...
	Prototype:
		oyster.ajax.close_popup()
	*/

	close_popup: function() {

		// hide the popup and background overlay
		$('oyster_ajax_popup').setStyle('display', 'none');
		$('oyster_ajax_popup_overlay').setStyle('display', 'none');

		// erase its contents
		oyster.ajax.send('', 'oyster_ajax_popup_content');
	...
	Prototype:
		oyster.ajax.submit_url(string url, string content_div_id)
	*/
	...
	submit_url: function(url, content_id) {
		...
		$('oyster_ajax_form').action = url;
		oyster.ajax.submit_form('oyster_ajax_form', content_id);
	},
	...
	Prototype:
		oyster.ajax.submit_form(string form_id, string content_id)
	*/
	...
	submit_form: function(form_id, content_id) {
		...
		form.target          = 'oyster_ajax_iframe';
	...
	send: function(xml, content_id) {
		...
		if (content_id == 'oyster_ajax_popup_content') {
			oyster.ajax.center_popup();
		}
	},
	...
	center_popup: function() {
		...
		var popup_width    = $('oyster_ajax_popup').getStyle('width').replace('px', ''); // remove px -- what happens if it's em?
		var horiz_padding  = browser_width / 2 - popup_width / 2;
		$('oyster_ajax_popup').setStyle('left', horiz_padding);

		var browser_height = window.getHeight();
		var popup_height   = $('oyster_ajax_popup').getStyle('height').replace('px', ''); // remove px -- what happens if it's em?
		var vert_padding   = browser_height / 2 - popup_height / 2;
		$('oyster_ajax_popup').setStyle('top', vert_padding);
	}
},
...
editbuttons: {
	...
	Prototype:
		oyster.editbuttons.install(string field_id)
	*/
	...
	install: function(field_id) {
		...
		$(translation_mode_field_id).addEvent('change', function() { oyster.editbuttons.populate(field_id) });

		// set initial edit buttons
		oyster.editbuttons.populate(field_id)
	},
	...
	Notes:
		* There must be a corresponding translation mode field with the name field_id_translation_mode
	Prototype:
		oyster.editbuttons.populate(string field_id)
	*/
	...
	populate: function(field_id) {
		...
		if ($(translation_mode_field_id).getValue() == 'bbcode') {
			$('oyster_editbuttons_' + field_id + '_bbcode').setStyle('display', 'block');
			$('oysters_editbuttons_' + field_id + '_xhtml').setStyle('display', 'none');
		}

		// xhtml translation mode
		if ($(translation_mode_field_id).getValue() == 'xhtml') {
			$('oyster_editbuttons_' + field_id + '_bbcode').setStyle('display', 'none');
			$('oyster_editbuttons_' + field_id + '_xhtml').setStyle('display', 'block');
		}
	},
	...
	Prototype:
		oyster.editbuttons.insert_tag(obj triggering_button, string field_id, string tag, bool wrap)
	*/
	...
	Prototype:
		oyster.editbuttons.insert_file(string field_id, int file_id)
	*/
