<xsl:template match="/oyster/user[@action = 'admin_groups_delete']" mode="heading">
	Delete Group - <xsl:value-of select="@name" />
</xsl:template>

<xsl:template match="/oyster/user[@action = 'admin_groups_delete']" mode="description">
	User groups are sets of permissions that can be applied to multiple users.
</xsl:template>

<xsl:template match="/oyster/user[@action = 'admin_groups_delete']" mode="content">
	<form id="admin_groups_delete" method="post" action="{/oyster/@url}?group={@id}">
		<dl>
